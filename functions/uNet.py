...
class DMUnet(nn.Module):
    def __init__(self, block):
        ...
        self.layerm1 = self._make_layer(block, 1, 8, blocks=1,stride=1)
        self.layer0 = self._make_layer(block, 8, 16, blocks=1,stride=2)
        self.layer1 = self._make_layer(block, 16, 16, blocks=2,stride=1)
        self.layer2 = self._make_layer(block, 16,32, blocks=1,stride=2)
        self.layer3 = self._make_layer(block,32,32,blocks=2,stride=1)
        self.layer4 = self._make_layer(block,32,64,blocks=1,stride=2)
        self.layer5 = self._make_layer(block,64,64,blocks=2,stride=1)
        self.layer6 = self._make_layer(block,64,128,blocks=1,stride=2)
        self.layer7 = self._make_layer(block,128,128,blocks=2,stride=1)
        self.deconv1 = nn.ConvTranspose3d(128,64,3,stride=2,padding=0)
        self.deconv_batchnorm1 = nn.BatchNorm3d(num_features = 64,momentum=0.1)
        self.layer8 = self._make_layer(block,160,80,blocks=1,stride=1)
        self.layer9 = self._make_layer(block,80,32,blocks=1,stride=1)
        self.deconv2 = nn.ConvTranspose3d(32,1,1,stride=1,padding=0)
        self.pad=nn.ConstantPad3d((0,1,0,1,0,1), 0)
        self.maxp=nn.MaxPool3d(2, stride=2)
...
class RegUnet(nn.Module):
    def __init__(self, block):
        ...
        self.layerm1 = self._make_layer(block, 1, 4, blocks=2,stride=1)
        self.layer0 = self._make_layer(block, 4, 8, blocks=1,stride=2)
        self.layer1 = self._make_layer(block, 8, 8, blocks=2,stride=1)
        self.layer2 = self._make_layer(block, 8,16, blocks=1,stride=2)
        self.layer3 = self._make_layer(block,16,16,blocks=2,stride=1)
        self.layer4 = self._make_layer(block,16,32,blocks=1,stride=2)
        self.layer5 = self._make_layer(block,32,32,blocks=2,stride=1)
        self.layer6 = self._make_layer(block,32,64,blocks=1,stride=2)
        self.layer7 = self._make_layer(block,64,64,blocks=2,stride=1)
        self.deconv1 = nn.ConvTranspose3d(64,32,3,stride=2,padding=0)
        self.deconv_batchnorm1 = nn.BatchNorm3d(num_features = 32,momentum=0.1)
        self.layer8 = self._make_layer(block,92,46,blocks=1,stride=1)
        self.layer9 = self._make_layer(block,46,23,blocks=1,stride=1)
        self.deconv2 = nn.ConvTranspose3d(23,1,1,stride=1,padding=0)
        self.pad=nn.ConstantPad3d((0,1,0,1,0,1), 0)
        self.maxp0=nn.MaxPool3d(8, stride=8)
        self.maxp1=nn.MaxPool3d(4, stride=4)
        self.maxp2=nn.MaxPool3d(2, stride=2)
    ...
    def forward(self,x):
        ...
        x  = self.pad(x)
        x  = F.leaky_relu(self.deconv_batchnorm1(crop_tensor(self.deconv1(x))),inplace=True)
        x  = torch.cat((x,xm1,x0,x1,x2),dim=1)
        x  = self.layer8(x)
        x  = self.layer9(x)
        x  = F.relu(self.deconv2(x),inplace=True)
